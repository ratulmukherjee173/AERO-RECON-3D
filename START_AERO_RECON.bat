...
if not errorlevel 1 (
    ...
    start /B python -m uvicorn backend.app.main:app --host 0.0.0.0 --port 8000 >nul 2>&1
    
    echo [INFO] Starting web server on http://localhost:4173 ...
    start /B python -m http.server 4173 --directory dist >nul 2>&1
) else (
    echo [INFO] Python not found. Backend will not start. Falling back to Node/Vite preview server...
